...
import pytest
from browser_tools_session import (
...
class TestAttachBrowserTool:
    ...
    def test_attach_does_not_launch_chrome(
        ...
        state = controller._ensure_browser_state()
    ...
    def test_project_preference_creates_headed_auth_profile(
        ...
        monkeypatch.setattr("persistent_browser.CACHE_DIR", tmp_path / "cache")
...
class TestAttachByProfile:
    ...
    def _setup_profile(self, monkeypatch, tmp_path: Path, fake_chrome: str, profile: str) -> Path:
        ...
        monkeypatch.setattr("persistent_browser.read_singleton_lock_pid", lambda d: 9999)
        monkeypatch.setattr("persistent_browser.is_process_alive", lambda pid: True)
        monkeypatch.setattr("persistent_browser.find_chrome_debug_port", lambda pid: port)
        monkeypatch.setattr(
            "persistent_browser.find_chrome_user_data_dir",
    ...
    def test_attach_profile_no_running_chrome_errors(self, monkeypatch, tmp_path: Path) -> None:
        ...
        monkeypatch.setattr("persistent_browser.read_singleton_lock_pid", lambda d: None)
    ...
    def test_attach_profile_dead_port_returns_actionable_error(
        ...
        monkeypatch.setattr("persistent_browser.CACHE_DIR", tmp_path)
        (tmp_path / "profiles" / "dev").mkdir(parents=True)
        monkeypatch.setattr("persistent_browser.read_singleton_lock_pid", lambda d: 4242)
        monkeypatch.setattr("persistent_browser.is_process_alive", lambda pid: True)
        monkeypatch.setattr("persistent_browser.find_chrome_debug_port", lambda pid: 9222)
        # is_devtools_available returns False — port is dead.
        monkeypatch.setattr("persistent_browser.is_devtools_available", lambda url: False)
        monkeypatch.setattr("persistent_browser.find_listeners_on_port", lambda p: [11111, 22222])
    ...
    def test_attach_endpoint_profile_mismatch_rejected(
        ...
        monkeypatch.setattr("persistent_browser.find_listeners_on_port", lambda p: [7777])
        monkeypatch.setattr(
            "persistent_browser.find_chrome_user_data_dir",
...
class TestListProfilesStatus:
    ...
    def test_list_profiles_reports_runtime_status(
        ...
        from browser_tools_session import handle_list_profiles
        ...
        monkeypatch.setattr("persistent_browser.read_singleton_lock_pid", fake_lock)
        monkeypatch.setattr("persistent_browser.is_process_alive", lambda pid: True)
        monkeypatch.setattr("persistent_browser.find_chrome_debug_port", lambda pid: port)
...
class TestLiveProfileFallback:
    ...
    def _setup_profiles(monkeypatch, tmp_path: Path, profiles: dict[str, int | None]):
        ...
        monkeypatch.setattr("persistent_browser.is_process_alive", lambda pid: True)
        monkeypatch.setattr(
            "persistent_browser.find_chrome_debug_port",
