"""JSON-RPC-over-stdio broker for the parity suite's Node engine.

The parity gate compares the native engine against the real
``chrome-devtools-mcp`` process, so the harness needs a multiplexer that owns
that subprocess and routes responses back to the caller that asked for them.
This is a test oracle, not a product surface: the MCP front is retired (#92),
and a production module must not stay alive only to serve a test, so the broker
lives with the suite that uses it.

Each request gets a fresh internal id; the matching response is routed back
through a per-id queue. A timeout never leaves a dangling pending entry, and a
response that arrives after its caller has timed out is dropped instead of
corrupting a later request.

The subprocess handle is injectable so the unit tests can drive the broker with
a fake process whose stdout is a text-iterable source the test feeds JSON lines
into and whose stdin captures writes, without spawning chrome-devtools-mcp.
"""
