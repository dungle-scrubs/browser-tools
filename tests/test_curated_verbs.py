...
class TestInstanceResolution:
    ...
    def test_a_known_failure_does_not_wait_out_the_deadline(self, monkeypatch, registry_path):
...
class TestHandlerToolDispatch:
    ...
    def test_screencast_starts_waits_and_stops_over_one_handler(self, registry_path, fake_handler):
...
def fake_screenshot_transport(monkeypatch):
    ...
    def _install(shot_data="Zm9v", targets=None):
        ...
        monkeypatch.setattr("browser_tools.one_shot.get_ws_url_async", _fake_get_ws_url_async)
        return calls
...
class TestSixVerbContracts:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ["wait-text", "Ready", "--timeout-ms", "321"],
                "wait-text",
...
def test_eval_throw_regression_at_cli(monkeypatch, fake_handler, capsys):
    ...
    assert (
        cli.main(
            [
                "eval",
                '(() => { throw new Error("regression-marker"); })()',
                "--endpoint",
                "http://127.0.0.1:9222",
            ]
        )
...
async def test_native_press_sends_down_and_up_and_type_one_insert():
    ...
    async def send(method, params=None, **kwargs):
        # The focus guard reads the active element. Answer it with a real
        # payload rather than a bare mock, so the guard runs its actual path
        # instead of falling through its unreadable-state escape.
        if method == "Runtime.evaluate":
            return {"result": {"value": "editable", "type": "string"}}
        return {}
    ...
    cdp.reset_mock()
    response = await handler._dispatch_curated_action(
        cdp, "type", {"text": "a\n🐈", "source": "text"}
    )
    # Two calls now: the focus guard reads the active element and is told an
    # editable one has focus, then the insert happens.
    sent = [call.args[0] for call in cdp.send.call_args_list]
    assert sent == ["Runtime.evaluate", "Input.insertText"], sent
    assert cdp.send.call_args_list[-1].args[1] == {"text": "a\n🐈"}
...
async def test_hover_uses_live_uid_box_and_native_mouse():
    ...
    point = await NativeInteractor(NativeSnapshotReader()).hover_async(
        send, f"{doc_token('f', 'l')}-3"
    )
    assert point == (20, 40)
    assert send.call_args_list[-2].args[0] == "DOM.getBoxModel"
    send.assert_awaited_with(
        "Input.dispatchMouseEvent", {"type": "mouseMoved", "x": 20, "y": 40, "buttons": 0}
    )


@pytest.mark.parametrize(
    ("size", "encoded", "omitted"),
    [
...
def test_network_body_limits_and_file_bytes(size, encoded, omitted, tmp_path):
    ...
    payload = {
        "body": base64.b64encode(data).decode() if encoded else data.decode(),
        "base64Encoded": encoded,
    }
    event = {
        "requestId": "1.2",
        "response": {"url": "https://a/api", "status": 200, "mimeType": "text/plain"},
    }
...
async def test_network_subscribes_before_enable_and_returns_without_fixed_wait(monkeypatch, delay):
    ...
    result = await asyncio.wait_for(
        curated_runtime.network_get(
            network_client(cdp), "/api", None, None, None, None, frozenset()
        ),
        0.5,
    )
...
async def test_network_unknown_request_id_explains_session_scope_and_cleans_up(monkeypatch, keep):
    ...
    with pytest.raises(LifecycleError, match="another invocation"):
        await curated_runtime.network_get(
            network_client(cdp), None, "old", None, None, None, keep, duration=0
        )
    assert "Network.disable" not in cdp.calls
    assert not any(cdp._handlers.values())
...
class TestCuratedInChrome:
    ...
    def test_hover_sets_css_rule(self, page):
        ...
        page("hover", "--uid", uid)
        assert (
            page("eval", 'getComputedStyle(document.querySelector("#h")).color')["value"]
            == "rgb(9, 8, 7)"
        )

    def test_type_refuses_when_nothing_editable_has_focus(self, drive, page):
        """A verb that dispatched anyway would report chars for a dropped insert."""
        page("eval", 'document.body.innerHTML="<input id=i2>"; document.activeElement.blur(); "ok"')
        message = drive("type", "zzz", status=1)
        assert "nothing editable has focus" in message
        assert "BODY" in message
        assert page("eval", 'document.getElementById("i2").value')["value"] == ""

    def test_type_still_reaches_a_focused_field(self, page):
        """The refusal must not cost the ordinary case."""
        page(
            "eval",
            'document.body.innerHTML="<input id=i2>"; document.getElementById("i2").focus(); "ok"',
        )
        assert page("type", "zzz")["chars"] == 3
        assert page("eval", 'document.getElementById("i2").value')["value"] == "zzz"

    def test_press_and_hover_documents_are_what_the_browser_returns(self, page):
        """These two were only ever asserted against a canned double's reply."""
        import re

        page("eval", 'document.querySelector("#i").focus()')
        pressed = page("press", "Enter")
        assert pressed == {"key": "Enter", "modifiers": [], "dispatched": ["keyDown", "keyUp"]}

        tree = page("snapshot")["snapshot"]
        uid = re.search(r'\[uid=([^\]]+)\] button "Hover me"', tree).group(1)
        hovered = page("hover", "--uid", uid)
        assert set(hovered) == {"uid", "x", "y"}
        assert hovered["uid"] == uid
        assert isinstance(hovered["x"], float) and isinstance(hovered["y"], float)
        assert hovered["x"] > 0 and hovered["y"] > 0
    ...
    def test_prose_inserts_at_caret_and_fires_input_not_keys(self, page, tmp_path):
        ...
        page(
            "eval",
            'document.body.innerHTML="<textarea id=i>AB</textarea>"; '
    ...
    def test_network_standalone_preserves_state_and_returns_on_match(self, page):
        ...
        page(
            "eval",
            "window.marker='kept'; setTimeout(()=>fetch('data:text/plain,delayed-body'), 500)",
        )
    ...
    def test_network_explicit_reload_fetches_response(self, page, tmp_path):
        ...
        try:
            ...
            saved = page(
                "network-get", "--url", "/api?first", "--reload", "--response-file", str(path)
            )
            assert saved["responseFile"] == str(path) and path.read_bytes() == b"/api?first"
            assert "another invocation" in page(
                "network-get", "--request-id", result["requestId"], "--duration", "0", status=1
            )
        finally:
            server.shutdown()
...
async def test_network_window_failure_paths(outcome, monkeypatch):
    ...
    with pytest.raises(
        LifecycleError,
        match={"none": "no matching", "unfinished": "did not finish", "failed": "net::ERR_FAILED"}[
            outcome
        ],
    ):
        await curated_runtime.network_get(
            network_client(cdp), "/api", None, None, None, None, frozenset(), duration=0
        )
    assert not any(cdp._handlers.values())
    assert "Network.disable" not in cdp.calls
...
async def test_wait_text_deadline_is_stderr_only_via_cli(monkeypatch, fake_handler, capsys):
    ...
    assert (
        cli.main(["wait-text", "never", "--timeout-ms", "1", "--endpoint", "http://127.0.0.1:9222"])
        == 1
    )
    captured = capsys.readouterr()
