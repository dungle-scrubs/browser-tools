...
class PersistentChromeController:
    ...
    def invoke_tool(self, tool_name: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        state = self._ensure_browser_state()
    ...
    def _ensure_browser_state(self) -> BrowserState:
        ...
        if lock_pid is not None and is_process_alive(lock_pid):
            raise MCPInvocationError(_format_dead_port_error(self.profile, user_data_dir, lock_pid))
...
def build_browser_command(
    *,
    ...
    command.append(INITIAL_PAGE_URL)
    return command
