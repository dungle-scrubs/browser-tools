...
def invoke_mcp_tool(
    ...
    try:
        ...
        raise MCPInvocationError("Tool invocation timed out after 60 seconds")
    except FileNotFoundError:
        raise MCPInvocationError("npx command not found. Please ensure Node.js is installed.")
    except Exception as e:
        raise MCPInvocationError(f"Unexpected error invoking tool: {e}")
...
def list_mcp_tools(config: list[str] | None = None) -> list[dict[str, Any]]:
    ...
    try:
        ...
        raise MCPInvocationError(f"Error listing tools: {e}")
...
def parse_viewport(viewport_str: str) -> tuple[int, int]:
    ...
    try:
        ...
        raise ParameterValidationError(
            f"Invalid viewport format '{viewport_str}'. Expected format: WIDTHxHEIGHT (e.g., 1280x720)"
        )
...
def parse_json_param(json_str: str, param_name: str) -> Any:
    ...
    try:
        ...
        raise ParameterValidationError(f"Invalid JSON for parameter '{param_name}': {e}")
