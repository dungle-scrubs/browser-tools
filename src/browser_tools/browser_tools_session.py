...
def create_persistent_controller(args, force_persistent=False):
...
def execute_screenshot_url(args):
    ...
    try:
        ...
        try:
            ...
            pass  # Page might not have visible "html" text, continue anyway
...
def execute_direct_tool(tool_name, params, args):
...
def execute_profiler_timed(args):
...
def execute_profiler_watch(args):
...
def handle_attach_browser(controller_ref: list, args: dict) -> dict:
    ...
    if endpoint is None:
        ...
        if not is_devtools_available(candidate):
            from .persistent_browser import _format_dead_port_error
            ...
            return {
                "result": {
                    "content": [
                        {
                            "type": "text",
                            "text": _format_dead_port_error(profile, profile_dir, lock_pid),
                        }
                    ],
                    "isError": True,
                }
    ...
    state = new_controller._ensure_browser_state()
    save_active_attach_config(
        endpoint,
        profile=profile,
        mode=mode,
...
def handle_delete_profile(args: dict) -> dict:
...
def handle_browser_session_status(args: dict) -> dict:
...
def handle_use_browser_session(controller_ref: list, args: dict) -> dict:
...
def _translate_args_for_camoufox(chrome_tool: str, args: dict) -> dict:
...
def _camoufox_result_to_mcp(result: dict) -> dict:
...
def choose_live_profile_fallback() -> tuple[PersistentChromeController | None, list[dict]]:
...
def select_default_controller() -> tuple[PersistentChromeController, list[dict] | None]:
...
def _format_live_profile_conflict_error(live: list[dict]) -> dict:
...
def create_tool_proxy_handlers():
    ...
    live_profile_conflict: list = [None]
    ...
    def call_tool(controller, tool, args):
        ...
        return active.invoke_tool(tool, args)
...
def main():
    ...
    tool_mappings = {
        ...
        "navigate": (
            "navigate_page",
            lambda: {"type": args.type, **(({"url": args.url}) if args.type == "url" else {})},
        ),
        "list-pages": ("list_pages", lambda: {}),
        ...
        "perf-stop": ("performance_stop_trace", lambda: {}),
    }
...
def _handle_launch_camoufox(camoufox_ref: list, args: dict) -> dict:
...
def _handle_close_camoufox(camoufox_ref: list) -> dict:
