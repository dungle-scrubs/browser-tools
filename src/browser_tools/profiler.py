...
class CDPProfiler:
    ...
    def __init__(self, ws_url: str):
        self.ws_url = ws_url
        self.ws = None  # websockets.ClientConnection (typed dynamically)
        self.msg_id = 0
    ...
    async def send(self, method: str, params: dict | None = None) -> dict:
        ...
        self.msg_id += 1
        msg: dict = {"id": self.msg_id, "method": method}
        if params:
            msg["params"] = params
...
def analyze_profile(profile: dict, top_n: int = 20) -> list[dict]:
    """
    ...
    results.sort(key=lambda x: x["selfTime"], reverse=True)
...
async def profile_page(duration: float = 5.0, port: int = 9222, format_type: str = "text") -> str:
    ...
    try:
        ...
        print(f"Starting profiler for {duration}s...", file=sys.stderr)
...
async def profile_until_high_cpu(
    ...
    try:
        ...
        print(f"Profiling... waiting for CPU > {threshold}% (timeout: {timeout}s)", file=sys.stderr)
        print("Trigger the freeze now!", file=sys.stderr)
                ...
                if metric["name"] == "TaskDuration":
                    ...
                    if last_timestamp is not None:
                        ...
                        if delta > 0.5:  # More than 500ms of task time in sample period
                            print(
                                f"High CPU detected! Capturing for {sample_window}s...",
                                file=sys.stderr,
                            )
        ...
        print("Timeout reached, returning profile...", file=sys.stderr)
        profile = await profiler.stop_profiling()
        results = analyze_profile(profile)
        return format_results(results, format_type)
